...
const schemaIE = z.optional(z.string()
    .superRefine((input, ctx) => {
        input = input.replace(/[\D]/g, '');
        if (input === "") return true
        if (input === "0".repeat(input.length)) return true;
    }));
const schemaEmail = z.optional(z.string({ required_error: 'Informe o E-mail' })
    .min(1, 'Informe o E-mail')
    .max(255, 'Máximo de 255 Caracteres')
    .email('E-mail Inválido')
    .toLowerCase());
export const schemaFornecedor = z.object({
    id_Fornecedor: z.string().uuid(),
    nr_Fornecedor: z.number(),
